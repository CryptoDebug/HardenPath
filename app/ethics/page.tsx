    ...
    <div className="mx-auto max-w-4xl px-4 py-5 sm:px-6 lg:px-8">
      <section className="hp-shell hp-atlas-surface hp-path-card p-6 sm:p-8">
        ...
        <div className="hp-inner">
          <span className="hp-checkpoint h-12 w-12 text-mint">
            <ShieldCheck aria-hidden className="h-7 w-7" />
          </span>
          <h1 className="hp-wrap mt-5 text-4xl font-black leading-tight text-white">{dictionary.ethics.title}</h1>
          ...
          <div className="mt-7 grid gap-4">
            {dictionary.ethics.principles.map((principle: string, index: number) => (
              <div className="hp-panel hp-route-card rounded-sm p-5 text-sm leading-7 text-slate-200" key={principle}>
                <div className="flex gap-3">
                  <span className="mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-sm bg-mint/[0.12] text-xs font-black text-mint">
                    {index + 1}
                  </span>
